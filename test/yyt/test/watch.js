    ...
    client
      ...
      .perform(async (done) => {
        if (index === 0) {
          ...
          await extFs.copyFiles(
            path.join(ORI_PATH, 'commons'),
        ...
        const htmls = await extFs.readFilePaths(distPath, (iPath) =>
          /\.html$/.test(iPath)
        )
        client.verify.ok(
          htmls.length !== 0,
          `build ${chalk.yellow.bold(htmls.length)} html files`
        )
        ...
        client.verify.ok(
          res.statusCode === 200,
          `${chalk.yellow('GET')} ${testUrl} ${chalk.green('200')}`
        )
      ...
      .perform(async (done) => {
        ...
        client.verify.ok(
          scssPaths.length !== 0,
          `expect have ${scssPaths.length} scss files: [${scssPaths[0]}]`
        )
      ...
      .getCssProperty('body', 'background-color', (result) => {
        client.verify.ok(
          result.value === 'rgba(255, 0, 0, 1)',
          `expect body turning red ${result.value}`
        )
      })
      // 环境关闭
      .end(async () => {
