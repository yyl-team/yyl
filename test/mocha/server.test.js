...
describe('server test', () => {
  ...
  it(
    ...
    util.makeAsync(async () => {
      const setting = await yyl.run(
        'server start --logLevel 0 --silent',
        __dirname
      )
  ...
  it(
    `yyl server start --path ${__dirname}`,
    util.makeAsync(async () => {
      const setting = await yyl.run(
        `server start --logLevel 0 --silent --path ${__dirname}`
      )
      expect(setting.localserver).not.equal(false)
      const testPath = `http://${vars.LOCAL_SERVER}:${setting.localserver.port}/server.test.js`
      const [, res] = await request(testPath)
  ...
  it(
    'yyl server start --path ./',
    util.makeAsync(async () => {
      const setting = await yyl.run(
        'server start --logLevel 0 --silent --path ./',
        __dirname
      )
      expect(setting.localserver).not.equal(false)
      const testPath = `http://${vars.LOCAL_SERVER}:${setting.localserver.port}/server.test.js`
