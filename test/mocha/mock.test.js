...
describe('mock test', () => {
  ...
  it(
    '/mockapi?_start=1&_end=3&_limit=2',
    util.makeAsync(async () => {
  ...
  it(
    '/mockapi?title_like=又',
    util.makeAsync(async () => {
      const testPath = `http://127.0.0.1:5000/mockapi?title_like=${encodeURIComponent(
        '又'
      )}`
      const argv = await get(testPath, true)
      const [res, data] = argv
  ...
  it(
    'jsonp test /mapi?jsonp=bb&bb=aa',
    util.makeAsync(async () => {
