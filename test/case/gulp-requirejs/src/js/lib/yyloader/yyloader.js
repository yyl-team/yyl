...
;(function () {
  ...
  var IS_IE =
    ...
    /(msie |Trident\/.*rv:)(\d*)/i.test(navigator.userAgent)
      ? RegExp.$2
      : false
        ...
        $.ajax({
          ...
          success: function (tpl) {
            if (!TEMPLATE_START.test(tpl) || !TEMPLATE_END.test(tpl)) {
              return done(
                'get tpl error, {$name} tpl is incomplete'.replace(
                  '{$name}',
                  param.name
                )
              )
  ...
  yyloader.setExprie = function (time) {
    ...
    for (var key in localData) {
      if (localData.hasOwnProperty(key) && now - localData[key].date > time) {
        delete localData[key]
        needUpdate = true
      }
    }
    if (needUpdate) {
      window.localStorage.setItem(LOCAL_STORAGE_NAME, JSON.stringify(localData))
    }
  }
