  ...
  switch (module) {
    ...
    case 'finish':
      ...
      if (cache.curType) {
        ...
        print.log.done(
          `${cache.curType} finished, total cost ${chalk.yellow.bold(
            print.fn.cost.format()
          )}, at ${chalk.yellow.green.bold(print.fn.timeFormat())}`
        )
      }
      cache.curType = null
      break
    ...
    case 'msg':
      ...
      if (cache.logLevel === 1 && cache.curType) {
        ...
        if (cache.curMark.create.length) {
          statusStr = `${statusStr} ${chalk.green('ADD')} ${chalk.yellow.bold(
            cache.curMark.create.length
          )}`
        }
        if (cache.curMark.update.length) {
          statusStr = `${statusStr} ${chalk.magenta('UPD')} ${chalk.yellow.bold(
            cache.curMark.update.length
          )}`
        }
        if (cache.curMark.del.length) {
          statusStr = `${statusStr} ${chalk.gray('DEL')} ${chalk.yellow.bold(
            cache.curMark.del.length
          )}`
        }
        if (cache.curMark.warn.length) {
          statusStr = `${statusStr} ${chalk.yellow('WARN')} ${chalk.yellow.bold(
            cache.curMark.warn.length
          )}`
        }
        if (cache.curMark.error.length) {
          statusStr = `${statusStr} ${chalk.red('ERR')} ${chalk.yellow.bold(
            cache.curMark.error.length
          )}`
        }
        ...
        if (iType === 'error') {
          ...
          const printType = function (type) {
            if (LOG_OPTION.type[cache.curInfo.type]) {
              return LOG_OPTION.type[type].color(
                ` ${LOG_OPTION.type[type].name} `
              )
            } else {
              return LOG_OPTION.type.info.color(
                ` ${LOG_OPTION.type.info.name} `
              )
            }
          }
          if (cache.curInfo.argv) {
            ...
            // eslint-disable-next-line max-len
            rArgv.unshift(
              `${printType(cache.curInfo.type)}${print.fn.makeSpace(
                10 - cache.curInfo.type.length
              )}${chalk.gray(
                LOADING_CHARS[cache.curInfo.padding % LOADING_CHARS.length]
              )}`
            )
          }
