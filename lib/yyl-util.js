    ...
    util = {
        ...
        buildTree: function(op){
            ...
            (function deep(iPath, parentStr){
                ...
                space = parentStr.split(/[-~]/)[0];
                space = space.replace('`', ' ');

                if(/\w/ig.test(parentStr)){
                    space += '  ';
                }
                ...
                list.forEach(function(filename, i){
                    var 
                        isDir = fs.statSync(path.join(iPath, filename)).isDirectory(),
                    ...
                    if(isDir && !noDeep){
                        deep(path.join(iPath, filename), rStr);
                    }

                });

            })(o.path, r.length && o.frontPath? r[r.length - 1]: space);
