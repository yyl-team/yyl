...
const seed = {
  ...
  async init({ logger }) {
    ...
    if (!fs.existsSync(pkgPath)) {
      ...
      logger('msg', ['info', [Lang.SeedConfigInitFinished]])
    }
  },
  ...
  async get({ seed, logger }) {
    const seedPath = path.join(SERVER_SEED_PATH, 'node_modules', seed)
    if (fs.existsSync(seedPath)) {
      return require(seedPath)
    } else {
      if (seed.packages.includes(seed)) {
        await seed.install({ cmds: seed, logger })
        return require(seedPath)
      } else {
        throw new Error(
