...
var 
    ...
    settings = [{
        ...
        commit: {
            // 上线配置
            revAddr: 'http://web.yystatic.com/project/proj01/pc/assets/proj01/rev-manifest.json',
...
function settingPrase(setting, obj){
    obj = {
...
if(settings.length === 1){
    settingPrase(settings[0], config);

} else {
    settings.forEach(function(setting){
        if(setting.name){
            config[setting.name] = {};
            settingPrase(setting, config[setting.name]);
        }
    });
